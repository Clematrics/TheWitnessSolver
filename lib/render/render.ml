...
let paths_layer style { graph; cuts_graph; ends; _ } =
  let is_cut p = (not (Graph.mem graph p)) && Graph.mem cuts_graph p in
  let rec synthetize_path complete_path graph =
    let path = start_path graph in
    let hole_size = (cell_size -. path_width) /. 5. in
    let rec iter_points ?(is_first = false) acc = function
      ...
      | [ c; c' ] when is_first ->
          ...
          let dir = V2.(p' - p) in
          iter_points
            (V2.(p' + (path_width /. cell_size * dir / norm dir))
            :: V2.(p - (path_width /. cell_size * dir / norm dir))
            :: acc)
            []
      | c :: c' :: l when is_first ->
    ...
    |> List.fold_left
         (fun path -> function
           ...
           | [] -> path)
         path
  and iter_from_path ({ completed; last_edge; _ } as path) graph =
    (* try to find the next edge in the same direction as last_edge *)
    (* possible edges to cross *)
    let candidate_edges = Graph.adjacent_edges (List.hd completed) graph in
    ...
    | Some edge ->
        ...
        (if Graph.mem graph next_point then
           iter_from_path
    ...
    | None ->
        ...
        synthetize_path completed graph
  and start_path graph =
    ...
    | Some (pos, _) ->
        ...
        let edge = Graph.adjacent_edges pos graph |> Edges.min_elt in
        let pos' = Edge.other_end edge pos in
        (* check path exists on the other size *)
        (if Graph.mem graph pos' then
           iter_from_path
             ({
                start_by_path = true;
                completed = [ pos'; pos ];
  ...
  in
  let path = start_path (Graph.union graph cuts_graph) in
...
let end_layer style { ends; graph; _ } =
  IntMap.fold
    (fun _ pos ->
      let pos' =
        Graph.adjacent_edges pos graph
        |> Edges.choose
